...
Adjust the paths to match your repo.

```
.
├── sql/
│   └── olist_data_audit_check.sql        # schema, audit, cleaning, KPI/KRI views, analysis queries
├── notebooks/
...
1. Install dependencies (use the same `scikit-learn` version the model was trained with, for example `scikit-learn==1.6.1`):
```
   pip install -r requirements.txt
```
2. Create `.streamlit/secrets.toml` with your Snowflake details (do not commit this file):
```toml
   [snowflake]
   user = "..."
   password = "..."
   ...
   schema = "..."
```
3. Make sure the 7 Olist tables exist in Snowflake and that `low_review_model.pkl` and `model_features.pkl` are next to `app.py`.
4. Start the app:
```
...
1. Download the dataset from Kaggle and load the 7 CSVs into Snowflake using Sections 1-2 of `olist_data_audit_check.sql`.
2. Run the audit, cleaning and views (Sections 3-5 and A-D) in the same script.
3. Run `Olist_Marketing_Analysis.ipynb`, then `Olist_predictive_modeling.ipynb` (it saves the model files).
